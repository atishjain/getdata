...
#Get data
path_to_file<-file.path(getwd(),"UCI HAR Dataset")

path_to_file_test<-file.path(path_to_file, "test")
path_to_file_train<-file.path(path_to_file, "train")
  
xtest<-read.table(file.path(path_to_file_test,"X_test.txt"))
ytest<-read.table(file.path(path_to_file_test,"Y_test.txt"))
subjecttest<-read.table(file.path(path_to_file_test,"subject_test.txt"))

xtrain<-read.table(file.path(path_to_file_train,"X_train.txt"))
ytrain<-read.table(file.path(path_to_file_train,"Y_train.txt"))
subjecttrain<-read.table(file.path(path_to_file_train,"subject_train.txt"))

#Get activity labels 
activitylabels<-read.table(file.path(path_to_file,
                              			"activity_labels.txt"),
                            col.names = c("Id", "Activity")
                            )

#Get features labels
featurelabels<-read.table(file.path(path_to_file,
                            		"features.txt"),
